...
<body>
	<main class="main_content_sm">
		<div class="content_label">
			<div class=" font_30">Q & A 글 수정</div>
			<div class="font_gray font_14">Q & A 글을 수정하세요.</div>
		</div>
		<form id="meeting_insert" action="/qna/update/${list.b_no }"
			...
			<div class="write_wrap padding_bottom_20">
				...
				<div class="input_section radio">
					...
					<c:if test="${list.topic_no eq 11}">checked</c:if> value="11">
					<label class="btn btn-outline-primary margin_right_20"
						for="vbtn-radio1">커리어</label> 
						
						
				    <input type="radio" class="btn-check" name="topic_no" id="vbtn-radio2" autocomplete="off"
					<c:if test="${list.topic_no eq 12}">checked</c:if> value="12">
					<label class="btn btn-outline-primary margin_right_20" for="vbtn-radio2">기술</label>
					
					
					
					<input type="radio" class="btn-check" name="topic_no"
						id="vbtn-radio3" autocomplete="off"
						<c:if test="${list.topic_no eq 13}">checked</c:if> value="13">
					<label class="btn btn-outline-primary margin_right_20" for="vbtn-radio3">알고리즘</label>
					
					
					<input type="radio" class="btn-check" name="topic_no"
						id="vbtn-radio4" autocomplete="off"
						<c:if test="${list.topic_no eq 14}">checked</c:if> value="14">
					<label class="btn btn-outline-primary" for="vbtn-radio4">기타</label>
